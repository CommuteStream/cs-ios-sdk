...
#import "CSMRAIDParser.h"
#import "CSMRAIDModalViewController.h"
#import "CSMRAIDServiceDelegate.h"
#import "CSMRAIDUtil.h"
#import "CSMRAIDSettings.h"
...
@interface CSMRAIDView () <UIWebViewDelegate, CSMRAIDModalViewControllerDelegate, UIGestureRecognizerDelegate>
{
    MRAIDState state;
    ...
    CSMRAIDOrientationProperties *orientationProperties;
    CSMRAIDResizeProperties *resizeProperties;
    
    CSMRAIDParser *mraidParser;
    CSMRAIDModalViewController *modalVC;
 ...
 rootViewController:(UIViewController *)rootViewController
{
    ...
    if (self) {
        ...
        orientationProperties = [[CSMRAIDOrientationProperties alloc] init];
        resizeProperties = [[CSMRAIDResizeProperties alloc] init];
        
        mraidParser = [[CSMRAIDParser alloc] init];
        ...
        htmlData = [CSMRAIDUtil processRawHtml:htmlData];
        if (htmlData) {
            [currentWebView loadHTMLString:htmlData baseURL:baseURL];
        } else {
            [CSLogger error:@"MRAID - View" withMessage:@"Ad HTML is invalid, cannot load"];
...
- (void)cancel
{
    [CSLogger debug:@"MRAID - View" withMessage:@"cancel"];
...
- (void)dealloc
{
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat: @"%@ %@", [self.class description], NSStringFromSelector(_cmd)]];
...
- (BOOL)isValidFeatureSet:(NSArray *)features
{
    ...
    for (id feature in features) {
        if (![kFeatures containsObject:feature]) {
            [CSLogger warning:@"MRAID - View" withMessage:[NSString stringWithFormat:@"feature %@ is unknown, no supports set", feature]];
            return NO;
        }
    }
    return YES;
...
- (void)setIsViewable:(BOOL)newIsViewable
{
    ...
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat:@"isViewable: %@", _isViewable?@"YES":@"NO"]];
}
...
- (void)setRootViewController:(UIViewController *)newRootViewController
{
    ...
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat:@"setRootViewController: %@", _rootViewController]];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    [CSLogger debug:@"MRAID - View" withMessage:@"self.frame has changed"];
    ...
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat:@"old %@", NSStringFromCGRect(oldFrame)]];
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat:@"new %@", NSStringFromCGRect(newFrame)]];
...
- (void)showAsInterstitial
{
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat: @"%@", NSStringFromSelector(_cmd)]];
    [self expand:nil];
}
...
- (void)close
{
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat: @"JS callback %@", NSStringFromSelector(_cmd)]];
...
{
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat: @"JS callback helper %@", NSStringFromSelector(_cmd)]];
...
{
    if(!bonafideTapObserved && CS_SUPPRESS_BANNER_AUTO_REDIRECT){
        [CSLogger info:@"MRAID - View" withMessage:@"Suppressing an attempt to programmatically call mraid.createCalendarEvent() when no UI touch event exists."];
    ...
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat: @"JS callback %@ %@", NSStringFromSelector(_cmd), eventJSON]];
    ...
    if ([supportedFeatures containsObject:MRAIDSupportsCalendar]) {
        ...
        [CSLogger warning:@"MRAID - View" withMessage:[NSString stringWithFormat:@"No calendar support has been included."]];
    }
}
...
{
    if(!bonafideTapObserved && CS_SUPPRESS_BANNER_AUTO_REDIRECT){
        [CSLogger info:@"MRAID - View" withMessage:@"Suppressing an attempt to programmatically call mraid.expand() when no UI touch event exists."];
        return;  // ignore programmatic touches (taps)
    }
    
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat: @"JS callback %@ %@", NSStringFromSelector(_cmd), (urlString ? urlString : @"1-part")]];
    ...
    modalVC = [[CSMRAIDModalViewController alloc] initWithOrientationProperties:orientationProperties];
    ...
    if (!urlString) {
        ...
        if (!error) {
            ...
            [CSLogger error:@"MRAID - View" withMessage:[NSString stringWithFormat:@"Could not load part 2 expanded content for URL: %@" ,urlString]];
            currentWebView = webView;
            webViewPart2.delegate = nil;
            webViewPart2 = nil;
            modalVC = nil;
...
{
    if(!bonafideTapObserved && CS_SUPPRESS_BANNER_AUTO_REDIRECT){
        [CSLogger info:@"MRAID - View" withMessage:@"Suppressing an attempt to programmatically call mraid.open() when no UI touch event exists."];
        return;  // ignore programmatic touches (taps)
    }
    
    urlString = [urlString stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat: @"JS callback %@ %@", NSStringFromSelector(_cmd), urlString]];
...
{
    if(!bonafideTapObserved && CS_SUPPRESS_BANNER_AUTO_REDIRECT){
        [CSLogger info:@"MRAID - View" withMessage:@"Suppressing an attempt to programmatically call mraid.playVideo() when no UI touch event exists."];
        return;  // ignore programmatic touches (taps)
    }
...
{
    if(!bonafideTapObserved && CS_SUPPRESS_BANNER_AUTO_REDIRECT){
        [CSLogger info:@"MRAID - View" withMessage:@"Suppressing an attempt to programmatically call mraid.resize when no UI touch event exists."];
        return;  // ignore programmatic touches (taps)
    }
...
- (void)setOrientationProperties:(NSDictionary *)properties;
{
    ...
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat: @"JS callback %@ %@ %@", NSStringFromSelector(_cmd), (allowOrientationChange ? @"YES" : @"NO"), forceOrientation]];
    orientationProperties.allowOrientationChange = allowOrientationChange;
    orientationProperties.forceOrientation = [CSMRAIDOrientationProperties MRAIDForceOrientationFromString:forceOrientation];
...
- (void)setResizeProperties:(NSDictionary *)properties;
{
    ...
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat: @"JS callback %@ %d %d %d %d %@ %@", NSStringFromSelector(_cmd), width, height, offsetX, offsetY, customClosePosition, (allowOffscreen ? @"YES" : @"NO")]];
    ...
    resizeProperties.customClosePosition = [CSMRAIDResizeProperties MRAIDCustomClosePositionFromString:customClosePosition];
...
{
    if(!bonafideTapObserved && CS_SUPPRESS_BANNER_AUTO_REDIRECT){
        [CSLogger info:@"MRAID - View" withMessage:@"Suppressing an attempt to programmatically call mraid.storePicture when no UI touch event exists."];
        return;  // ignore programmatic touches (taps)
    }
    ...
    if ([supportedFeatures containsObject:MRAIDSupportsStorePicture]) {
        ...
        [CSLogger warning:@"MRAID - View" withMessage:[NSString stringWithFormat:@"No MRAIDSupportsStorePicture feature has been included"]];
    }
}
...
- (void)useCustomClose:(NSString *)isCustomCloseString
{
    ...
    [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat: @"JS callback %@ %@", NSStringFromSelector(_cmd), (isCustomClose ? @"YES" : @"NO")]];
...
- (void)fireSizeChangeEvent
{
    @synchronized(self){
        ...
        // [CSLogger debug:[NSString stringWithFormat:@"orientation is %@", (isLandscape ?  @"landscape" : @"portrait")]];
...
- (BOOL)webView:(UIWebView *)wv shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if ([scheme isEqualToString:@"mraid"]) {
        ...
        [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat:@"JS console: %@",
                                                     [[absUrlString substringFromIndex:14] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding ]]];
    } else {
        [CSLogger info:@"MRAID - View" withMessage:[NSString stringWithFormat:@"Found URL %@ with type %@", absUrlString, @(navigationType)]];
        
        // Links, Form submissions
        if (navigationType == UIWebViewNavigationTypeLinkClicked) {
            // For banner views
            if ([self.delegate respondsToSelector:@selector(mraidViewNavigate:withURL:)]) {
                [CSLogger debug:@"MRAID - View" withMessage:[NSString stringWithFormat:@"JS webview load: %@",
...
- (void)initWebView:(UIWebView *)wv
{
    ...
    if ([supportedFeatures containsObject:MRAIDSupportsInlineVideo]) {
        ...
        [CSLogger warning:@"MRAID - View" withMessage:[NSString stringWithFormat:@"No inline video support has been included, videos will play full screen without autoplay."]];
    }
...
- (void)parseCommandUrl:(NSString *)commandUrlString
{
    ...
    if (!commandDict) {
        [CSLogger warning:@"MRAID - View" withMessage:[NSString stringWithFormat:@"invalid command URL: %@", commandUrlString]];
        return;
    }
...
{
    if(!CS_SUPPRESS_BANNER_AUTO_REDIRECT){
...
-(void)oneFingerOneTap
{
    ...
    [CSLogger debug:@"MRAID - View" withMessage:@"tapGesture oneFingerTap observed"];
}

- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    if (touch.view == resizeCloseRegion || touch.view == closeEventRegion){
        [CSLogger debug:@"MRAID - View" withMessage:@"tapGesture 'shouldReceiveTouch'=NO"];
        return NO;
    }
    [CSLogger debug:@"MRAID - View" withMessage:@"tapGesture 'shouldReceiveTouch'=YES"];
    return YES;
}
