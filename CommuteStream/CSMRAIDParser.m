...
- (NSDictionary *)parseCommandUrl:(NSString *)commandUrl;
{
    ...
    [CSLogger debug:@"MRAID - Parser" withMessage:[NSString stringWithFormat:@"%@ %@", NSStringFromSelector(_cmd), commandUrl]];
    ...
    if (![self isValidCommand:command]) {
        [CSLogger warning:@"MRAID - Parser" withMessage:[NSString stringWithFormat:@"command %@ is unknown", command]];
        return nil;
    }
    
    // Check for valid parameters for the given command.
    if (![self checkParamsForCommand:command params:params]) {
        [CSLogger warning:@"MRAID - Parser" withMessage:[NSString stringWithFormat:@"command URL %@ is missing parameters", commandUrl]];
        return nil;
    }
